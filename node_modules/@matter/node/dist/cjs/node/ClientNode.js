...
class ClientNode extends import_Node.Node {
  ...
  initialize() {
    ...
    import_ClientCacheBuffer.ClientCacheBuffer.configure(this, store);
  ...
  get owner() {
    ...
    if (owner === void 0) {
      throw new import_general.InternalError("Client node is missing owner");
    }
    return super.owner;
  }
  set owner(owner) {
  ...
  async delete() {
    ...
    await super.delete();
    if (address) {
      await this.env.maybeGet(import_protocol.PeerSet)?.get(address)?.delete();
    }
  }
  async erase() {
  ...
  get interaction() {
    if (this.#interaction === void 0) {
      if (this.#blockInteractions) {
        throw new import_general.ImplementationError("Cannot access interaction of a shutting-down node");
      }
  ...
  get peerAddress() {
    let address = (0, import_protocol.PeerAddress)(this.behaviors.maybeStateOf("commissioning")?.peerAddress);
    if (!address) {
      address = (0, import_protocol.PeerAddress)(this.store.storeForEndpoint(this).peerAddress);
    }
    return address;
  }
  get identity() {
    ...
    if (peerAddress) {
      return (0, import_protocol.PeerAddress)(peerAddress).toString();
    }
