...
class ServerNetworkRuntime extends import_NetworkRuntime.NetworkRuntime {
  ...
  async addTransports(interfaces) {
    ...
    for (let attempt = 1; attempt <= maxPortRetries; attempt++) {
      let ipv6Interface;
      try {
        ipv6Interface = await import_general.UdpInterface.create(
          network,
          "udp6",
          port ? port : void 0,
      ...
      if (netconf.ipv4) {
        try {
          ipv4Interface = await import_general.UdpInterface.create(
            network,
            "udp4",
            ipv6Interface.port,
          ...
          if (error instanceof import_general.AddressInUseError && attempt < maxPortRetries) {
            logger.info(
  ...
  async start() {
    ...
    const exchanges = env.get(import_protocol.ExchangeManager);
    ...
    env.get(import_protocol.ScannerSet);
    const { timing, profiles } = this.owner.state.network;
    if (timing) {
      env.get(import_protocol.PeerSet).timing = timing;
    }
    const autoUnknown = this.#detectFallbackProfile();
    const effectiveProfiles = autoUnknown !== void 0 ? { unknown: autoUnknown, ...profiles } : profiles;
    if (effectiveProfiles) {
      env.get(import_protocol.NetworkProfiles).defaults = effectiveProfiles;
    }
    env.get(import_protocol.PeerSet).exchanges = exchanges;
    this.abortSignal.addEventListener(
      "abort",
      () => {
        this.owner.env.maybeGet(import_InteractionServer.InteractionServer)?.blockNewActivity();
        this.#subscriptionsDrained = this.owner.env.maybeGet(import_protocol.ClientSubscriptions)?.blockNewActivity();
      },
      { once: true }
    );
  ...
  async stop() {
    ...
    if (this.#subscriptionsDrained) {
      var _stack = [];
      try {
        const _lifetime = __using(_stack, this.construction.join("subscription reads"));
    ...
    }
    {
      var _stack2 = [];
      try {
        const _lifetime = __using(_stack2, this.construction.join("commissioner"));
    ...
    this.#mdnsAdvertiser = void 0;
    {
      var _stack3 = [];
      try {
        const _lifetime = __using(_stack3, this.construction.join("preparing"));
    ...
    this.#groupNetworking = void 0;
    {
      var _stack4 = [];
      try {
        const _advertiser = __using(_stack4, this.construction.join("advertisement"));
    ...
    }
    {
      var _stack5 = [];
      try {
        const _lifetime = __using(_stack5, this.construction.join("peers"));
        await env.maybeGet(import_protocol.PeerSet)?.disconnect();
      } catch (_5) {
    ...
    }
    {
      var _stack6 = [];
      try {
        const _lifetime = __using(_stack6, this.construction.join("exchanges"));
    ...
    }
    {
      var _stack7 = [];
      try {
        const _lifetime = __using(_stack7, this.construction.join("protocols"));
    ...
    }
    {
      var _stack8 = [];
      try {
        const _lifetime = __using(_stack8, this.construction.join("transports"));
        await env.maybeGet(import_general.ConnectionlessTransportSet)?.close();
      } catch (_8) {
    ...
    }
    {
      var _stack9 = [];
      try {
        const _lifetime = __using(_stack9, this.construction.join("interactions"));
   ...
   */
  #detectFallbackProfile() {
    const { owner } = this;
    let hasApplicationEndpoint = false;
    for (const part of owner.parts) {
      if (!import_model.DeviceClassification.isUtility(part.type.deviceClass)) {
    ...
    const nc = owner.behaviors.typeFor(import_network_commissioning.NetworkCommissioningBehavior);
    if (nc?.schema.supportedFeatures.has("TH")) {
      logger.info("Default network profile for unknown peers set to thread");
      return import_protocol.NetworkProfiles.defaults.thread;
    }
    logger.info("Default network profile for unknown peers set to fast");
    return import_protocol.NetworkProfiles.defaults.fast;
  }
