...
var import_ota_software_update_requestor2 = require("@matter/types/clusters/ota-software-update-requestor");
/**
 * @license
...
class OtaAnnouncements {
  ...
  async #announceOtaProvider(peerAddress) {
    ...
    if (existingOtaProviderRecord === void 0 || !(0, import_general.isDeepEqual)(consideredOtaProviderRecord, existingOtaProviderRecord)) {
      try {
          ...
          await peer.interaction.write(
            (0, import_protocol.Write)(
              import_protocol.Write.Attribute({
                ...
                cluster: import_ota_software_update_requestor2.OtaSoftwareUpdateRequestor,
  ...
  async announceOtaProvider(endpoint, peerAddress, announcementReason = import_ota_software_update_requestor2.OtaSoftwareUpdateRequestor.AnnouncementReason.SimpleAnnouncement) {
    try {
      ...
      if ((0, import_general.causedBy)(error, import_general.ClosedError)) {
        logger.debug(
          `Node ${(0, import_protocol.PeerAddress)(peerAddress)}/ep${endpoint.number} closed connection during OTA announcement, will retry later`
        );
      } else {
        logger.info(
          `Failed to notify node ${(0, import_protocol.PeerAddress)(peerAddress)}/ep${endpoint.number} about available OTA update:`,
          error
        );
      }
    }
