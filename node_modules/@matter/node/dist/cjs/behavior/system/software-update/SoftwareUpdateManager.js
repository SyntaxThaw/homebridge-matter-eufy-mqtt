...
var SoftwareUpdateManager_exports = {};
__export(SoftwareUpdateManager_exports, {
  OtaUpdateStatus: () => OtaUpdateStatus,
  SoftwareUpdateManager: () => SoftwareUpdateManager
});
module.exports = __toCommonJS(SoftwareUpdateManager_exports);
var import_Behavior = require("#behavior/Behavior.js");
var import_DclBehavior = require("#behavior/system/dcl/DclBehavior.js");
var import_OtaAnnouncements = require("#behavior/system/software-update/OtaAnnouncements.js");
...
class SoftwareUpdateManager extends import_Behavior.Behavior {
  ...
  async initialize() {
    const rootNode = import_Node.Node.forEndpoint(this.endpoint);
    rootNode.behaviors.require(import_DclBehavior.DclBehavior);
    this.internal.otaService = rootNode.agentFor(this.context).get(import_DclBehavior.DclBehavior).otaUpdateService;
  ...
  async #nodeOnline() {
    ...
    this.internal.announcements = new import_OtaAnnouncements.OtaAnnouncements(this.endpoint, ownFabric);
    this.#updateAnnouncementSettings();
    const delay = (0, import_general.Millis)((0, import_general.Seconds)(Math.floor(Math.random() * 300)) + (0, import_general.Minutes)(5));
    logger.info(`Scheduling first OTA update check in ${import_general.Duration.format(delay)}`);
    this.internal.checkForUpdateTimer?.stop();
    this.internal.checkForUpdateTimer = import_general.Time.getTimer(
  ...
  async #initializeUpdateCheck() {
    ...
    this.internal.checkForUpdateTimer.stop();
    this.internal.checkForUpdateTimer = import_general.Time.getPeriodicTimer(
  ...
  get queuedUpdates() {
    ...
    return this.internal.updateQueue.map((entry) => {
      ...
      if (entry.lastProgressUpdateTime === void 0) {
        status = "queued";
      } else if (entry.lastProgressUpdateTime + (0, import_general.Minutes)(15) < now) {
        status = "stalled";
      } else {
        status = "in-progress";
      }
  ...
  async updateExistsFor(peerAddress, { softwareVersion, vendorId, productId, requestorCanConsent }) {
    ...
    if (candidate) {
      ...
      return {
        ...
        fileDesignator: new import_protocol.FileDesignator(`ota/${filename}`),
  ...
  async #cleanupObsoleteUpdates() {
    ...
    for (const peer of rootNode.peers) {
      const basicInfo = peer.maybeStateOf(import_basic_information.BasicInformationClient);
      if (basicInfo === void 0) {
        continue;
      }
  ...
  #preparePeerForUpdate(peer) {
    ...
    const { vendorId, productId, softwareVersion, softwareVersionString } = peer.stateOf(import_basic_information.BasicInformationClient);
    const versionEvent = peer.eventsOf(import_basic_information.BasicInformationClient).softwareVersion$Changed;
    ...
    const startUpEvent = peer.eventsOf(import_basic_information.BasicInformationClient).startUp;
    if (!this.internal.versionUpdateObservers.observes(startUpEvent)) {
      let triggerStartUp2 = function({ softwareVersion: softwareVersion2 }, context) {
        that.#onSoftwareVersionChanged(
          peerAddress,
          softwareVersion2,
          true,
          (0, import_protocol.hasRemoteActor)(context) ? context.session : void 0
        );
      };
   ...
   */
  #onSoftwareVersionChanged(peerAddress, newVersion, isStartUp = false, currentSession) {
    ...
    if (isStartUp) {
      ...
      if (suppressedSessionId !== void 0) {
        this.internal.pendingStartUpSuppress.delete(peerAddress.toString());
        const sessionId = currentSession?.id ?? this.env.get(import_protocol.SessionManager).maybeSessionFor(peerAddress)?.id;
    ...
    if (newVersion < expectedVersion) {
      if (isStartUp && (previousProgressStatus === 5 /* Applying */ || previousProgressStatus === 4 /* WaitForApply */)) {
        logger.warn(
  ...
  #triggerQueuedUpdate() {
    ...
    if (inProgressCount === 0 && this.internal.updateQueueTimer?.isRunning) {
      ...
      for (const entry of inProgressEntries) {
        if (entry.lastProgressUpdateTime + (0, import_general.Minutes)(15) < now) {
          logger.info(
    ...
    if (nextEntry) {
      ...
      if (!this.internal.updateQueueTimer?.isRunning) {
        this.internal.updateQueueTimer = import_general.Time.getPeriodicTimer(
          "checkQueuedUpdates",
  ...
  async #triggerUpdateOnNode(entry) {
    ...
    if (this.env.get(import_protocol.BdxProtocol).sessionFor(peerAddress, this.storage.scope) !== void 0) {
    ...
    try {
      ...
      await this.internal.announcements.announceOtaProvider(
        endpoint,
        peerAddress,
        import_ota_software_update_requestor.OtaSoftwareUpdateRequestor.AnnouncementReason.UpdateAvailable
      );
      if (this.internal.updateQueue.indexOf(entry) >= 0) {
        entry.lastProgressUpdateTime = import_general.Time.nowMs;
      }
    } catch (error) {
  ...
  async forceUpdate(peerAddress, vendorId, productId, targetSoftwareVersion) {
    ...
    if (!added) {
      throw new import_protocol.OtaUpdateError(`Node at ${peerAddress.toString()} is not currently applicable for OTA updates`);
    }
    ...
    if (!entry) {
      throw new import_general.InternalError(`Failed to find queued update after adding consent`);
    }
  ...
  async addUpdateConsent(peerAddress, vendorId, productId, targetSoftwareVersion) {
    ...
    if (otaEndpoint !== void 0 && (nodeVendorId !== vendorId || nodeProductId !== productId)) {
      throw new import_general.ImplementationError(`Node at ${peerAddress.toString()} does not match given vendorId/productId`);
    }
    consents.push({
      vendorId,
      productId,
   ...
   */
  suppressNextStartUp(peerAddress, sessionId) {
...
((SoftwareUpdateManager2) => {
  class State {
    ...
    updateCheckInterval = (0, import_general.Hours)(24);
    ...
    announcementInterval = (0, import_general.Hours)(24);
  }
  ...
  class Internal {
    ...
    updateQueueTimer;
    announcements;
    versionUpdateObservers = new import_general.ObserverGroup();
     ...
     */
    pendingStartUpSuppress = /* @__PURE__ */ new Map();
  }
  ...
  class Events extends import_general.EventEmitter {
    ...
    updateAvailable = (0, import_general.Observable)();
    /** Emitted when an update for a Peer is finished */
    updateDone = (0, import_general.Observable)();
    /** Emitted when an update for a Peer has failed or was cancelled */
    updateFailed = (0, import_general.Observable)();
    announceAsDefaultProvider$Changed = (0, import_general.Observable)();
    announcementInterval$Changed = (0, import_general.Observable)();
  }
