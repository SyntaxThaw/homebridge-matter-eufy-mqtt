...
function ClusterBehaviorType({
  ...
  if (!schema) {
    throw new import_general.ImplementationError("ClusterBehaviorType: no schema available");
  }
  if (features !== void 0) {
  ...
  if (!namespace) {
    namespace = (0, import_types.ClusterType)(schema);
  }
  const useCache = name === void 0;
  if (useCache) {
    const cached = import_ClusterBehaviorCache.ClusterBehaviorCache.get(base, schema, forClient);
    if (cached) {
      return cached;
    }
...
function createDerivedState({ scope, base, newProps }) {
  ...
  for (const name in props) {
    ...
    if (propSchema.id === import_model.FeatureMap.id) {
      defaults[name] = computeFeatureFlags(schema);
      continue;
    }
...
function schemaForId(id) {
  for (const child of import_model.Matter.children) {
    if (child.tag === import_model.ElementTag.Cluster && child.id === id) {
      return child;
    }
  }
  return void 0;
...
function syncFeatures(schema, namespace) {
  let incomingFeatures;
  const nsSupportedFeatures = namespace?.supportedFeatures;
  if (nsSupportedFeatures) {
    incomingFeatures = new import_model.FeatureSet(nsSupportedFeatures);
  } else {
    return schema;
  }
...
function applyFeatureSelection(schema, features) {
  if (features === true) {
    const featureSet2 = new import_model.FeatureSet();
    for (const feature of schema.features) {
      featureSet2.add(feature.name);
    }
    return syncFeaturesFromSet(schema, featureSet2);
  }
  const featureSet = new import_model.FeatureSet();
  const model = schema;
...
function syncFeaturesFromSet(schema, featureSet) {
...
function computeFeatureFlags(schema) {
  const flags = {};
  for (const child of schema.featureMap.children) {
    const key = (0, import_general.camelize)(child.title ?? child.name);
...
function createDefaultCommandDescriptors({ scope, base, commandFactory }) {
  ...
  const names = new Set(
    scope.membersOf(scope.owner, { tags: [import_model.ElementTag.Command] }).map((command) => command.propertyName)
  );
  const conformantNames = new Set(
    scope.membersOf(scope.owner, { tags: [import_model.ElementTag.Command], conformance: "conformant" }).map((command) => command.propertyName)
  );
