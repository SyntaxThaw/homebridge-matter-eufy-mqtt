...
class Endpoint {
  ...
  set number(number) {
    ...
    if (this.type.deviceClass === import_root.RootEndpoint.deviceClass) {
      ...
      if (this.lifecycle.isInstalled) {
        this.env.get(import_IdentityService.IdentityService).assertEndpointNumberAvailable(number, this);
      }
    }
  ...
  get path() {
    ...
    return new import_model.DataModelPath(this.identity, this.type?.name);
  }
  /**
   * Diagnostic identity.
   *
  ...
  get [import_general.Diagnostic.value]() {
    const diagnostics = [
      import_general.Diagnostic.strong(this.#id === void 0 ? "(no id)" : this.id),
      import_general.Diagnostic.dict({
        ...this.#diagnosticProps,
        class: this.constructor.name
      })
    ];
    if (this.#construction.status === import_general.Lifecycle.Status.Active) {
      diagnostics.push(import_general.Diagnostic.list([...this.behaviors.detailedDiagnostic, ...this.parts]));
    } else {
      diagnostics.push(import_general.Diagnostic.weak(`(${this.#construction.status})`));
    }
    return diagnostics;
  }
