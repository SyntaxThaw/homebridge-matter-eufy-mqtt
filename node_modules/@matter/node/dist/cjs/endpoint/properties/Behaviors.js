...
class Behaviors {
    ...
    return Object.entries(this.#supported).map(([name, type]) => {
      ...
      const result = [
        ...
        import_general.Diagnostic.dict({
          id: cluster?.id !== void 0 ? import_general.Diagnostic.hex(cluster.id) : void 0
        })
      ];
      if (!cluster) {
        return result;
      ...
      const features = schema instanceof import_model.ClusterModel ? schema.supportedFeatures : new import_model.FeatureSet();
      if (features.size) {
        ...
        elementDiagnostic.push([
          ...
          import_general.Diagnostic.list(
            [...elements.commands].map((name2) => {
              const command = cluster.commands?.[name2];
              if (!command) {
                return [name2];
              }
              ...
              return [
                ...
                import_general.Diagnostic.weak(
                  `(${import_general.Diagnostic.hex(command.id)}${response ? `/${import_general.Diagnostic.hex(response.id)}` : ""})`
                )
              ];
            })
          )
        ...
        elementDiagnostic.push(
          ...
          import_general.Diagnostic.list([
            [...elements.events].map((name2) => {
              const event = cluster.events?.[name2];
              return event ? [name2, import_general.Diagnostic.weak(`(${import_general.Diagnostic.hex(event.id)})`)] : [name2];
            })
          ])
        );
      }
  ...
  inject(type, options, notify = true) {
    ...
    if (!/^[a-z]/.test(type.id)) {
      throw new import_general.ImplementationError(
        `Behavior ID "${type.id}" must start with a lowercase letter (for example "${(0, import_general.camelize)(type.id)}")`
      );
    }
    if (options) {
  ...
  defaultsFor(type) {
    ...
    const { variableService } = this.#endpoint.env.get(import_EndpointInitializer.EndpointInitializer);
    if (variableService) {
