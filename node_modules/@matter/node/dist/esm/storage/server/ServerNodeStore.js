...
class ServerNodeStore extends NodeStore {
  ...
  async bdxStore() {
    if (!this.#bdxHandle) {
      const root = this.#env.has(DatafileRoot) ? this.#env.get(DatafileRoot) : void 0;
      const blobNamespace = root ? new DatafileRoot(root.directory.directory(`${this.#nodeId}-bdx`)) : `${this.#nodeId}-bdx`;
