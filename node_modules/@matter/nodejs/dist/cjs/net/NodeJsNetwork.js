...
class NodeJsNetwork extends import_general.Network {
  ...
  static getMembershipMulticastInterfaces(netInterfaceOrZone, ipv4) {
    if (ipv4) {
      return [void 0];
    } else {
      const multicastInterfaces = Object.entries((0, import_node_os.networkInterfaces)()).flatMap(([netIf, netIfInfo]) => {
        if (netIfInfo === void 0) {
          return [];
        }
        ...
        if (netInterfaceOrZone !== void 0 && netInterfaceOrZone !== zone) {
          return [];
        }
        return [`::%${zone}`];
      });
      if (multicastInterfaces.length === 0) {
        logger.warn(
          `No IPv6 multicast interface found${netInterfaceOrZone !== void 0 ? ` for interface ${netInterfaceOrZone}` : ""}.`
        );
      }
