  ...
  #processMessage(message) {
    ...
    if (packetRelevant) {
      for (let record of filtered) {
        if (record.recordType !== import_DnsCodec.DnsRecordType.A && record.recordType !== import_DnsCodec.DnsRecordType.AAAA || this.has(record.name)) {
          continue;
        }
        const key = record.name.toLowerCase();
        if (record.ttl === 0) {
          const staged2 = this.#stagedIpRecords.get(key);
          if (staged2 === void 0) {
            continue;
          }
          const remaining = staged2.filter(
        ...
        if (existing === -1) {
          staged.push({ record, receivedAt: import_Time.Time.nowMs });
        } else {
          staged[existing] = { record, receivedAt: import_Time.Time.nowMs };
        }
        this.#stagedIpRecords.delete(key);
