...
function Read(optionsOrSelector, ...selectors) {
  ...
  const result = {
    ...
    [Diagnostic.value]: () => Diagnostic.dict({
      attributes: attributeRequests?.length ? selectors.length ? selectors.filter(({ kind }) => kind === "attribute").map((path) => resolvePathForSpecifier(path)).join(", ") : attributeRequests.length : void 0,
      events: eventRequests?.length ? selectors.length ? selectors.filter(({ kind }) => kind === "event").map((path) => resolvePathForSpecifier(path)).join(", ") : eventRequests.length : void 0,
