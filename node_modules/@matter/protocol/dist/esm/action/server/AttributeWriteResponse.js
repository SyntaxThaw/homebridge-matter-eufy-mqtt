...
import { DataModelPath, ElementTag, FabricIndex as FabricIndexField } from "@matter/model";
import {
  ArraySchema,
  FabricIndex,
  Status,
...
class AttributeWriteResponse extends DataResponse {
  ...
  #writeAttributeForWildcard(attribute, path, value) {
    ...
    if (hasRemoteActor(this.session)) {
      if (this.session.authorityAt(attribute.limits.writeLevel, this.#guardedCurrentCluster.location) !== AccessControl.Authority.Granted) {
        return;
      }
  ...
  #asStatus(path, status, clusterStatus) {
    if (status !== Status.Success) {
      logger.debug(
        () => `Error writing attribute ${this.node.inspectPath(path)}: Status=${StatusCode[status]}(${toHex(status)}), ClusterStatus=${clusterStatus !== void 0 ? toHex(clusterStatus) : void 0}`
      );
    }
    const report = {
      kind: "attr-status",
