...
class BtpMatterError extends MatterError {
}
...
class BtpSessionHandler {
  ...
  ackReceiveTimer = Time.getTimer(
    "BTP ack timeout",
    MatterBle.BTP_ACK_TIMEOUT,
  ...
  sendAckTimer = Time.getTimer(
    "BTP send timeout",
    MatterBle.BTP_SEND_ACK_TIMEOUT,
    () => this.btpSendAckTimeoutTriggered()
  );
  isActive = true;
  idleTimeout = Time.getTimer("Central Device Idle Timer", MatterBle.BTP_CONN_IDLE_TIMEOUT, async () => {
  ...
  static async createFromHandshakeRequest(maxDataSize, handshakeRequestPayload, writeBleCallback, disconnectBleCallback, handleMatterMessagePayload) {
    ...
    const version = MatterBle.BTP_SUPPORTED_VERSIONS.find((version2) => versions.includes(version2));
    if (version === void 0) {
