...
class PeerAddressMonitor {
  ...
  async #check() {
    ...
    if (await interaction.probe({ network: probeNetwork.id, abort: this.#abort })) {
      ...
      logger.debug(via, `Probe succeeded, keeping session (next cooldown: ${import_general.Duration.format(nextCooldown)})`);
      return;
    }
