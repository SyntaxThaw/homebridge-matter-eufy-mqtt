...
__export(ControllerCommissioningFlow_exports, {
  ...
  FabricLabelConflictError: () => FabricLabelConflictError,
  MaximumCommissionedFabricsReachedError: () => MaximumCommissionedFabricsReachedError,
  NodeIdConflictError: () => NodeIdConflictError,
  OperativeConnectionFailedError: () => OperativeConnectionFailedError,
  ThreadNetworkSetupFailedError: () => ThreadNetworkSetupFailedError,
  WifiNetworkSetupFailedError: () => WifiNetworkSetupFailedError,
...
var import_Write = require("#action/request/Write.js");
...
var import_descriptor = require("@matter/types/clusters/descriptor");
var import_general_commissioning = require("@matter/types/clusters/general-commissioning");
var import_network_commissioning = require("@matter/types/clusters/network-commissioning");
var import_operational_credentials = require("@matter/types/clusters/operational-credentials");
var import_ota_software_update_requestor = require("@matter/types/clusters/ota-software-update-requestor");
var import_time_synchronization = require("@matter/types/clusters/time-synchronization");
...
const BTP_IDLE_ALIVE_INTERVAL = (0, import_general.Seconds)(25);
...
class ControllerCommissioningFlow {
    ...
    for (const step of this.commissioningSteps) {
      ...
      try {
        ...
        if (this.#currentFailSafeEndTime !== void 0) {
          ...
          const timeLeft = (0, import_general.Timespan)(import_general.Time.nowMs, this.#currentFailSafeEndTime).duration;
          if (timeLeft < this.#defaultFailSafeTime / 2) {
            logger.info(
              `After Commissioning step ${step.stepNumber}.${step.subStepNumber}: ${step.name} succeeded, ${import_general.Duration.format(timeLeft)} left for failsafe timer, re-arming failsafe`
            );
            await this.#armFailsafe();
        ...
        if (error instanceof RecoverableCommissioningError) {
          logger.warn(
            ...
            import_general.Diagnostic.weak("(continuing commissioning)")
          );
        } else if ((0, import_general.causedBy)(error, import_CommissioningError.CommissioningError, import_types.StatusResponseError)) {
          logger.error(
            `Commissioning step ${step.stepNumber}.${step.subStepNumber}: ${step.name} failed with error:`,
            import_general.Diagnostic.errorMessage(error),
            import_general.Diagnostic.weak("(terminating commissioning)")
          );
  ...
  /** Helper method to check for errorCode/debugTest responses and throw error on failure */
    ...
    logger.debug(
      `Commissioning step ${context} returned ${import_operational_credentials.OperationalCredentials.NodeOperationalCertStatus[statusCode]} (${statusCode}), ${debugText}${fabricIndex !== void 0 ? `, fabricIndex: ${fabricIndex}` : ""}`
    );
    if (statusCode === import_operational_credentials.OperationalCredentials.NodeOperationalCertStatus.Ok) return;
    if (context === "addNoc") {
      if (statusCode === import_operational_credentials.OperationalCredentials.NodeOperationalCertStatus.FabricConflict) {
      ...
      } else if (statusCode === import_operational_credentials.OperationalCredentials.NodeOperationalCertStatus.TableFull) {
      ...
      } else if (statusCode === import_operational_credentials.OperationalCredentials.NodeOperationalCertStatus.LabelConflict) {
  ...
  async #getInitialData() {
    const [
      ...
      (0, import_Read.Read)(
        import_Read.Read.Attribute({
          ...
          cluster: import_operational_credentials.OperationalCredentials,
          attributes: ["supportedFabrics", "commissionedFabrics"]
        }),
        import_Read.Read.Attribute({
          endpoint: RootEndpointNumber,
          cluster: import_descriptor.Descriptor,
          attributes: ["partsList", "serverList"]
        }),
        import_Read.Read.Attribute({
          endpoint: RootEndpointNumber,
          cluster: import_basic_information.BasicInformation,
          attributes: ["vendorId", "productId", "productName"]
        }),
        import_Read.Read.Attribute({
          endpoint: RootEndpointNumber,
          cluster: import_general_commissioning.GeneralCommissioning,
    ...
    const networkData = this.interaction.read(
      (0, import_Read.Read)(
        import_Read.Read.Attribute({
          cluster: import_network_commissioning.NetworkCommissioning,
          attributes: ["featureMap", "networks"]
        }),
        import_Read.Read.Attribute({
          cluster: import_ota_software_update_requestor.OtaSoftwareUpdateRequestor,
    ...
    for await (const data of networkData) {
      for (const entry of data) {
        ...
        switch (attributeId) {
          case import_types.Global.attributes.featureMap.id:
            networkFeatures.push({
              endpointId,
              value
            });
            break;
          case import_network_commissioning.NetworkCommissioning.attributes.networks.id:
            networkStatus.push({
              endpointId,
              value
            });
            break;
          case import_ota_software_update_requestor.OtaSoftwareUpdateRequestor.attributes.defaultOtaProviders.id:
  ...
  async #armFailsafe(time) {
    if (this.collectedCommissioningData.basicCommissioningInfo === void 0) {
      ...
      this.#commissioningExpiryTime = (0, import_general.Timestamp)(
        this.#commissioningStartedTime + (0, import_general.Seconds)(basicCommissioningInfo.maxCumulativeFailsafeSeconds)
      );
    }
  ...
  async #ensureFailsafeTimerFor(maxProcessingTime) {
    ...
    if (this.interaction.channelType === import_general.ChannelType.BLE) {
      ...
      this.#armFailsafeInterval = import_general.Time.getPeriodicTimer(
        ...
        () => {
          ...
          if (this.#commissioningExpiryTime !== void 0 && now < this.#commissioningExpiryTime) {
            logger.debug(
              `Re-Arm Failsafe Timer during longer actions with device. Time left: ${import_general.Duration.format((0, import_general.Timespan)(now, this.#commissioningExpiryTime).duration)}`
            );
    ...
    if (timeLeft < minFailsafeTime) {
      logger.debug(
        `Failsafe timer has only ${import_general.Duration.format(timeLeft)} left, re-arming for at least ${import_general.Duration.format(minFailsafeTime)}`
      );
      await this.#armFailsafe(import_general.Duration.max(minFailsafeTime, this.#defaultFailSafeTime));
    } else {
      logger.debug(`Failsafe timer is already set for at least ${import_general.Duration.format(timeLeft)}`);
    }
  }
  ...
  async #configureNetworkThread() {
    ...
    if (!this.commissioningOptions.threadNetwork?.networkName) {
      ...
      if (networkingStatus2 !== import_network_commissioning.NetworkCommissioning.NetworkCommissioningStatus.Success) {
    ...
    if (addNetworkingStatus !== import_network_commissioning.NetworkCommissioning.NetworkCommissioningStatus.Success) {
      throw new ThreadNetworkSetupFailedError(
        `Commissionee failed to add Thread network: status=${import_network_commissioning.NetworkCommissioning.NetworkCommissioningStatus[addNetworkingStatus]} (${addNetworkingStatus})${addDebugText ? ` ${addDebugText}` : ""}`
      );
    }
    if (networkIndex === void 0) {
    ...
    if (connected) {
      logger.debug(
        `Commissionee is already connected to Thread network ${this.commissioningOptions.threadNetwork.networkName ?? "via operational dataset"} (networkId ${import_general.Bytes.toHex(networkId)})`
      );
      return {
        code: 0 /* Success */,
        breadcrumb: this.lastBreadcrumb
    ...
    if (networkingStatus !== import_network_commissioning.NetworkCommissioning.NetworkCommissioningStatus.Success) {
      throw new ThreadNetworkSetupFailedError(
        `Commissionee failed to connect to Thread network: status=${import_network_commissioning.NetworkCommissioning.NetworkCommissioningStatus[networkingStatus]} (${networkingStatus})${debugText ? ` ${debugText}` : ""}`
      );
    }
    logger.debug(
      `Commissionee successfully connected to Thread network ${this.commissioningOptions.threadNetwork.networkName ?? "via operational dataset"} (networkId ${import_general.Bytes.toHex(networkId)})`
    );
    return {
      code: 0 /* Success */,
      breadcrumb: this.lastBreadcrumb
  ...
  async #reconnectWithDevice() {
    ...
    await this.#ensureFailsafeTimerFor((0, import_general.Minutes)(5));
    if (!isConcurrentFlow) {
      this.#armFailsafeInterval?.stop();
    }
    ...
    for (const requestorEndpoint of this.collectedCommissioningData.otaRequestorList) {
      try {
        const writeResult = await this.interaction.write(
          (0, import_Write.Write)(
            import_Write.Write.Attribute({
        ...
        import_action.WriteResult.assertSuccess(writeResult);
        success = true;
