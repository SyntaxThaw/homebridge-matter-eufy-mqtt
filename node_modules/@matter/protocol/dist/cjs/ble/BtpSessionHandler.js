...
class BtpSessionHandler {
  ...
  ackReceiveTimer = import_general.Time.getTimer(
    "BTP ack timeout",
    import_BleConsts.MatterBle.BTP_ACK_TIMEOUT,
  ...
  sendAckTimer = import_general.Time.getTimer(
    "BTP send timeout",
    import_BleConsts.MatterBle.BTP_SEND_ACK_TIMEOUT,
    () => this.btpSendAckTimeoutTriggered()
  );
  isActive = true;
  idleTimeout = import_general.Time.getTimer("Central Device Idle Timer", import_BleConsts.MatterBle.BTP_CONN_IDLE_TIMEOUT, async () => {
  ...
  static async createFromHandshakeRequest(maxDataSize, handshakeRequestPayload, writeBleCallback, disconnectBleCallback, handleMatterMessagePayload) {
    ...
    const version = import_BleConsts.MatterBle.BTP_SUPPORTED_VERSIONS.find((version2) => versions.includes(version2));
    if (version === void 0) {
      await disconnectBleCallback();
      throw new BtpProtocolError(`No supported BTP version found in ${versions}`);
    }
    let attMtu = import_BleConsts.MatterBle.MINIMUM_ATT_MTU;
    if (maxDataSize !== void 0) {
      if (maxDataSize > import_BleConsts.MatterBle.MINIMUM_ATT_MTU) {
        if (handshakeMtu <= import_BleConsts.MatterBle.MINIMUM_ATT_MTU) {
          attMtu = Math.min(maxDataSize, import_BleConsts.MatterBle.MAXIMUM_BTP_MTU);
        } else {
          attMtu = Math.min(handshakeMtu, maxDataSize, import_BleConsts.MatterBle.MAXIMUM_BTP_MTU);
        }
      }
    }
    const fragmentSize = attMtu;
    const windowSize = Math.min(import_BleConsts.MatterBle.BTP_MAXIMUM_WINDOW_SIZE, clientWindowSize);
  ...
  static async createAsCentral(handshakeResponsePayload, writeBleCallback, disconnectBleCallback, handleMatterMessagePayload) {
    ...
    const fragmentSize = Math.min(handshakeMtu, import_BleConsts.MatterBle.MAXIMUM_BTP_MTU);
  ...
  async processSendQueue() {
    ...
    while (this.queuedOutgoingMatterMessages.length > 0) {
      ...
      try {
        ...
        import_Ble.BleDisconnectedError.accept(error);
        logger.debug("BTP packet send failed because BLE is disconnected", import_general.Diagnostic.errorMessage(error));
  ...
  async btpSendAckTimeoutTriggered() {
    ...
    if (this.prevIncomingSequenceNumber > this.prevAckedSequenceNumber) {
      ...
      try {
        ...
        import_Ble.BleDisconnectedError.accept(error);
        logger.debug("BTP ACK send failed because BLE is disconnected", import_general.Diagnostic.errorMessage(error));
        return;
      }
      this.prevAckedSequenceNumber = this.prevIncomingSequenceNumber;
      if (!this.ackReceiveTimer.isRunning) {
