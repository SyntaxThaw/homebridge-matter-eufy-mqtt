...
class MessageChannel {
  ...
  async send(message, options) {
    ...
    if (addressOverride && this.#isIpNetworkChannel) {
      logContext = {
        ...logContext,
        address: import_general.ServerAddress.urlFor(addressOverride)
      };
    }
    ...
    if (addressOverride && this.#isIpNetworkChannel) {
      return await this.#channel.send(bytes, addressOverride);
    }
  ...
  set socket(channel) {
    ...
    const addressChanged = !(0, import_general.sameIpNetworkChannel)(channel, this.#channel);
    this.#observeChannelAddress(channel);
    this.#channel = channel;
    if (addressChanged) {
  ...
  #observeChannelAddress(newChannel) {
    if (this.#channelAddressObserver && this.#isIpNetworkChannel) {
      this.#channel.networkAddressChanged.off(this.#channelAddressObserver);
    }
    this.#channelAddressObserver = (networkAddress) => {
  ...
  async close() {
    ...
    if (this.#channelAddressObserver && this.#isIpNetworkChannel) {
      this.#channel.networkAddressChanged.off(this.#channelAddressObserver);
      this.#channelAddressObserver = void 0;
    }
