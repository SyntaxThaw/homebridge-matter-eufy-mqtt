...
class MessageExchange {
  ...
  async send(messageType, payload, options = {}) {
    if (this.#closeCause) {
      throw new import_general.ClosedError("Exchange is closed", { cause: this.#closeCause });
    }
    ...
    try {
      ...
      if (this.#messageReceivedCounter === 0 && (0, import_general.causedBy)(e, import_PeerCommunicationError.TransientPeerCommunicationError, import_general.TimeoutError, import_general.NetworkError)) {
        await this.#context.peerLost(this, (0, import_general.asError)(e));
      }
      throw e;
    }
  }
  async #send(messageType, payload, standaloneAckMessageId) {
    ...
    try {
      ...
      const abort = __using(_stack2, new import_general.Abort(this.#sendOptions));
      ...
      if (standaloneAckMessageId !== void 0) {
        ...
        if (messageType !== import_types.SecureMessageType.StandaloneAck) {
          throw new import_general.InternalError("Cannot override an acknowledge message ID for non-standalone acks");
        }
      }
      ...
      const protocolId = isStandaloneAck ? import_types.SECURE_CHANNEL_PROTOCOL_ID : this.#protocolId;
      if (isStandaloneAck) {
        if (!this.session.usesMrp) {
          return;
        }
      ...
      if (this.session.usesMrp && message.payloadHeader.requiresAck && !disableMrpLogic) {
        ...
        logContext.backOff = import_general.Duration.format(backOff);
        ...
        const startedWaitingAt = import_general.Time.nowMs;
        this.#sentMessageAckFailure = () => {
          abort.abort(new import_PeerCommunicationError.PeerUnresponsiveError(import_general.Timestamp.delta(startedWaitingAt)));
        };
      }
      ...
      if (ackPromise !== void 0) {
        ...
        try {
          ...
          if (abort.aborted) {
            this.#retransmissionTimer?.stop();
            if (!(0, import_general.causedBy)(abort.reason, import_general.AbortedError)) {
              throw abort.reason;
            }
            return;
          }
  ...
  async nextMessage(options) {
    try {
      ...
      if (this.#messageReceivedCounter === 0 && (0, import_general.causedBy)(e, import_PeerCommunicationError.TransientPeerCommunicationError)) {
        await this.#context.peerLost(this, (0, import_general.asError)(e));
      }
      throw e;
    }
  ...
  async #nextMessage(options) {
    ...
    try {
      ...
      const localAbort = __using(_stack, new import_general.Abort({
        ...
        abort: options?.abort,
        timeoutHandler: () => {
          throw new import_PeerCommunicationError.PeerUnresponsiveError(timeout);
        }
      }));
  ...
  async sendStandaloneAckForMessage(message) {
    ...
    await this.#send(import_types.SecureMessageType.StandaloneAck, new Uint8Array(0), messageId);
  }
  #retransmitMessage(message, expectedProcessingTime) {
    this.#retransmissionCounter++;
    this.#totalRetransmissionCounter++;
    if (this.considerClosed || this.#retransmissionCounter >= (this.#sendOptions.maxRetransmissions ?? import_MRP.MRP.MAX_TRANSMISSIONS)) {
  ...
  get #mrpResubmissionBackOffTime() {
    ...
    if (this.#sendOptions.initialRetransmissionTime !== void 0) {
      backOff = (0, import_general.Millis)(backOff + this.#sendOptions.initialRetransmissionTime);
    }
    return import_general.Duration.min(backOff, this.#sendOptions.maxRetransmissionTime ?? import_general.Forever);
  }
}
