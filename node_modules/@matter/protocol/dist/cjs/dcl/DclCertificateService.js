...
var DclCertificateService_exports = {};
__export(DclCertificateService_exports, {
  DclCertificateService: () => DclCertificateService
});
module.exports = __toCommonJS(DclCertificateService_exports);
var import_general = require("@matter/general");
var import_AttestationCertificates = require("../certificate/kinds/AttestationCertificates.js");
 ...
 */
const logger = import_general.Logger.get("DclCertificateService");
class DclCertificateService {
  ...
  constructor(environment, options = {}) {
    ...
    logger.info(
      ...
      import_general.Diagnostic.dict({
        prod: options.dclConfig?.url ?? import_DclConfig.DclConfig.production.url,
        test: options.fetchTestCertificates ? options.testDclConfig?.url ?? import_DclConfig.DclConfig.test.url : void 0,
    ...
    this.#construction = (0, import_general.Construction)(this, async () => {
      this.#storageManager = await environment.get(import_general.StorageService).open("certificates");
  ...
  #normalizeSubjectKeyId(subjectKeyId) {
    ...
    return import_general.Bytes.toHex(subjectKeyId).toUpperCase();
  }
  /**
  ...
  async getCertificateAsPem(subjectKeyId) {
    ...
    if (!metadata) {
      throw new import_DclClient.MatterDclError(`Certificate not found`, import_general.Diagnostic.dict({ skid: normalizedId }));
    }
    const derBytes = await this.#storage.get(normalizedId);
    if (!derBytes) {
      throw new import_DclClient.MatterDclError(`Certificate data not found in storage`, import_general.Diagnostic.dict({ skid: normalizedId }));
    }
    return import_general.Pem.encode(derBytes);
  }
  /**
  ...
  async getOrFetchCertificate(subjectKeyId, options) {
    ...
    try {
      ...
      const config = isProduction ? this.#options.dclConfig ?? import_DclConfig.DclConfig.production : this.#options.testDclConfig ?? import_DclConfig.DclConfig.test;
      ...
      if (!certRef) {
        logger.debug(
          ...
          import_general.Diagnostic.dict({ skid: normalizedId, prod: isProduction })
        );
        return;
      }
  ...
  async deleteCertificate(subjectKeyId) {
    ...
    await this.#saveIndex();
    logger.debug(`Deleted certificate`, import_general.Diagnostic.dict({ skid: normalizedId }));
  }
  /**
    ...
    for (const metadata of storedIndex) {
      if (allKeys.delete(metadata.subjectKeyId)) {
        ...
        logger.info(
          ...
          import_general.Diagnostic.dict({ skid: metadata.subjectKeyId })
        );
        invalidCount++;
      }
    }
  ...
  #saveIndex() {
    ...
    if (!this.#storage) {
      throw new import_DclClient.MatterDclError("Storage context not initialized");
    }
      ...
      if (!force && this.#certificateIndex.has(normalizedSubjectKeyId)) {
        ...
        if (isProduction && !existing.isProduction) {
          ...
          logger.debug(
            ...
            import_general.Diagnostic.dict({ skid: normalizedSubjectKeyId })
          );
        } else {
          logger.debug(
      ...
      for (const cert of certs) {
        if (!cert.subjectKeyId) {
          logger.warn(
            ...
            import_general.Diagnostic.dict({ subject: cert.subject })
          );
          continue;
        }
        const subjectKeyId = this.#normalizeSubjectKeyId(cert.subjectKeyId);
        const derBytes = import_general.Pem.asDer(cert.pemCert);
        ...
        logger.debug(`Stored certificate`, import_general.Diagnostic.dict({ skid: normalizedSubjectKeyId, vid: cert.vid }));
      }
    } catch (error) {
  ...
  async #fetchGitHubCertificates(storage, force) {
    try {
      ...
      const { owner, repo, branch, certPath } = this.#options.githubConfig ?? import_DclConfig.DclGithubConfig.defaults;
      const repoClient = new import_general.Github.Repo(owner, repo, branch, this.#options);
  ...
  async #fetchGitHubCertificate(storage, certDir, filename, force) {
    try {
      ...
      const paa = import_AttestationCertificates.Paa.fromAsn1(derBytes);
      ...
      if (!force && this.#certificateIndex.has(subjectKeyId)) {
        logger.debug(`Certificate already exists, skipping`, import_general.Diagnostic.dict({ skid: subjectKeyId }));
        return;
      }
      const subject = filename.replace(".der", "");
      const serialNumber = import_general.Bytes.toHex(paa.cert.serialNumber);
      ...
      logger.debug(`Stored GitHub certificate`, import_general.Diagnostic.dict({ skid: subjectKeyId, filename }));
    } catch (error) {
