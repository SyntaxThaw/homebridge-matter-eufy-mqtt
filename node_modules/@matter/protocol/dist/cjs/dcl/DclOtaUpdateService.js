...
var DclOtaUpdateService_exports = {};
__export(DclOtaUpdateService_exports, {
  DclOtaUpdateService: () => DclOtaUpdateService,
  OtaUpdateError: () => OtaUpdateError
});
module.exports = __toCommonJS(DclOtaUpdateService_exports);
var import_PersistedFileDesignator = require("#bdx/PersistedFileDesignator.js");
var import_ScopedStorage = require("#bdx/ScopedStorage.js");
var import_DclRestApiTypes = require("#dcl/DclRestApiTypes.js");
var import_general = require("@matter/general");
var import_types = require("@matter/types");
var import_OtaImageReader = require("../ota/OtaImageReader.js");
 ...
 */
const logger = import_general.Logger.get("DclOtaUpdateService");
class OtaUpdateError extends import_general.MatterError {
  constructor(message, options) {
    super(message, options);
  }
}
const OTA_DOWNLOAD_TIMEOUT = (0, import_general.Minutes)(5);
const OTA_FILENAME_REGEX = /^[0-9a-f]+[./][0-9a-f]+[./](?:prod|test|local)[./]\d+$/i;
class DclOtaUpdateService {
  #construction;
  #crypto;
  #options;
  #blobDriver;
  ...
  constructor(environment, options) {
    ...
    this.#crypto = environment.get(import_general.Crypto);
    this.#options = options;
    logger.info(
      "Initialize OTAUpdateService",
      import_general.Diagnostic.dict({
        prod: options?.productionDclConfig?.url ?? import_DclConfig.DclConfig.production.url,
        test: options?.testDclConfig?.url ?? import_DclConfig.DclConfig.test.url
      })
    );
    this.#construction = (0, import_general.Construction)(this, async () => {
      const blobHandle = await environment.get(import_general.StorageService).openBlobStorage("ota");
      this.#blobDriver = blobHandle.driver;
      this.#closeBlobStorage = blobHandle.close;
      this.#storage = new import_ScopedStorage.ScopedStorage(this.#blobDriver, ["bin"], "ota");
      ...
      for (const productHex of await blobDriver.contexts(vendorContexts)) {
        ...
        for (const key of keys) {
          ...
          try {
            ...
            const header = await import_OtaImageReader.OtaImageReader.header(headerReader);
  ...
  async #queryDclForUpdate(options) {
    ...
    const config = isProduction ? this.#options?.productionDclConfig ?? import_DclConfig.DclConfig.production : this.#options?.testDclConfig ?? import_DclConfig.DclConfig.test;
    const dclClient = new import_DclClient.DclClient(config);
    ...
    try {
      if (targetSoftwareVersion !== void 0) {
        logger.debug(
      ...
      const newerVersions = softwareVersions.filter((version) => version > currentSoftwareVersion).sort((a, b) => b - a);
      if (newerVersions.length === 0) {
        logger.debug(`No newer versions available in ${dclLogStr}`, import_general.Diagnostic.dict(diagnosticInfo));
        return;
      }
      logger.debug(
        `Found ${newerVersions.length} newer version(s) in ${dclLogStr}`,
        import_general.Diagnostic.dict(diagnosticInfo)
      );
        ...
        try {
          ...
          if (updateInfo !== void 0) {
            logger.info(
              ...
              import_general.Diagnostic.dict({ new: version, ...diagnosticInfo })
            );
            return updateInfo;
          }
        } catch (error) {
          import_DclClient.MatterDclError.accept(error);
          logger.info(`Failed to check for update for VID: ${vendorId}, PID: ${productId}: ${error.message}`);
        }
      }
      logger.debug(`No applicable updates found in ${dclLogStr}`, import_general.Diagnostic.dict(diagnosticInfo));
    } catch (error) {
      import_DclClient.MatterDclError.accept(error);
  ...
  async checkForUpdate(options) {
    ...
    if (includeStoredUpdates) {
      ...
      for (let i = localUpdates.length - 1; i >= 0; i--) {
        ...
        const localUpdate = {
          ...entry,
          vid: (0, import_types.VendorId)(vendorId),
          pid: productId,
          cdVersionNumber: 0,
        ...
        if (localUpdate.softwareVersion > currentSoftwareVersion && currentSoftwareVersion >= localUpdate.minApplicableSoftwareVersion && currentSoftwareVersion <= localUpdate.maxApplicableSoftwareVersion) {
          logger.debug(`Found applicable local update`, import_general.Diagnostic.dict(localUpdate));
          if (targetSoftwareVersion !== void 0 && localUpdate.softwareVersion === targetSoftwareVersion) {
            return localUpdate;
          }
    ...
    if (isProduction !== false) {
      ...
      if (prodUpdate !== void 0) {
        const updateEntry = {
          ...prodUpdate,
          source: "dcl-prod"
        };
        if (targetSoftwareVersion !== void 0 && updateEntry.softwareVersion === targetSoftwareVersion) {
          return updateEntry;
        }
    ...
    if (targetSoftwareVersion !== void 0 || !foundUpdates.length) {
      return;
    }
    foundUpdates.sort((a, b) => b.softwareVersion - a.softwareVersion);
    return foundUpdates[0];
  }
  async #verifyUpdate(updateInfo, fileDesignator) {
    ...
    const checksumOptions = updateInfo.otaChecksum ? {
      ...
      checksumType: import_general.HashFipsAlgorithmId[updateInfo.otaChecksumType ?? 1],
      expectedChecksum: updateInfo.otaChecksum
    } : void 0;
    const header = await import_OtaImageReader.OtaImageReader.file(reader, this.#crypto, otaFileSize, checksumOptions);
  ...
  #fileName(vid, pid, mode, softwareVersion) {
   ...
   */
  async store(stream, updateInfo, mode = true) {
    ...
    const fileDesignator = new import_PersistedFileDesignator.PersistedFileDesignator(filename, storage);
    const diagnosticInfo = {
      filename,
      vid,
      pid,
    ...
    try {
      ...
      logger.debug(`Stored OTA image`, import_general.Diagnostic.dict(diagnosticInfo));
   ...
   */
  async downloadUpdate(updateInfo, force = false, timeout = OTA_DOWNLOAD_TIMEOUT) {
    if (this.#storage === void 0) {
      await this.construction;
    }
    const storage = this.#storage;
    const { otaUrl, softwareVersion, softwareVersionString, vid, pid, source } = updateInfo;
    const mode = source === "dcl-prod" ? "prod" : source === "dcl-test" ? "test" : "local";
    const diagnosticInfo = {
      vid,
      pid,
      v: `${softwareVersion} (${softwareVersionString})`,
    ...
    if (await fileDesignator.exists()) {
      if (!force) {
        try {
          await this.#verifyUpdate(updateInfo, fileDesignator);
          logger.info(`Existing OTA image validated successfully`, import_general.Diagnostic.dict(diagnosticInfo));
          return fileDesignator;
        } catch (error) {
    ...
    logger.info(`Downloading OTA image from ${otaUrl}`, import_general.Diagnostic.dict(diagnosticInfo));
    try {
      ...
      import_general.MatterError.reject(error);
   ...
   */
  async #checkSpecificVersion(dclClient, vendorId, productId, softwareVersion, currentVersion) {
   ...
   */
  #isVersionApplicable(versionInfo, currentVersion) {
    ...
    if (versionInfo.minApplicableSoftwareVersion !== void 0 && currentVersion < versionInfo.minApplicableSoftwareVersion) {
      return false;
    }
    if (versionInfo.maxApplicableSoftwareVersion !== void 0 && currentVersion > versionInfo.maxApplicableSoftwareVersion) {
      return false;
    }
    return true;
  }
   ...
   */
  async updateInfoFromStream(stream, otaUrl, options) {
    const reader = stream.getReader();
    const header = await import_OtaImageReader.OtaImageReader.header(reader);
    const updateInfo = {
      vid: (0, import_types.VendorId)(header.vendorId),
      ...
      minApplicableSoftwareVersion: header.minApplicableSoftwareVersion ?? options?.minApplicableSoftwareVersion ?? 0,
      maxApplicableSoftwareVersion: header.maxApplicableSoftwareVersion ?? options?.maxApplicableSoftwareVersion ?? header.softwareVersion - 1,
   ...
   */
  async createUpdateInfoFromFile(fileUrl, options) {
  ...
  async find(options = {}) {
    ...
    const modeOrder = { prod: 0, test: 1, local: 2 };
    results.sort((a, b) => {
  ...
  async #findEntries(baseContexts, options) {
    const blobDriver = this.#blobDriver;
    ...
    for (const vendorIdHex of await blobDriver.contexts(baseContexts)) {
      const vendorId2 = parseInt(vendorIdHex, 16);
      ...
      result.push(
        ...vendorEntries.map((entry) => ({
          ...entry,
          vendorId: vendorId2,
          filename: this.#fileName(vendorId2, entry.productId, entry.mode, entry.softwareVersion)
        }))
      );
    }
    return result;
  }
  async #findVendorEntries(vendorContexts, options) {
    const blobDriver = this.#blobDriver;
    ...
    for (const productIdHex of await blobDriver.contexts(vendorContexts)) {
      const productId2 = parseInt(productIdHex, 16);
      const productContexts = [...vendorContexts, productIdHex];
      const productEntries = await this.#findVendorProductEntries(productContexts, options);
      result.push(...productEntries.map((entry) => ({ ...entry, productId: productId2 })));
    }
    return result;
  }
  async #findVendorProductEntries(productContexts, options) {
    const blobDriver = this.#blobDriver;
    const { isProduction, mode: filterMode } = options;
    const result = new Array();
    const modeSubContexts = await blobDriver.contexts(productContexts);
    const validModes = ["prod", "test", "local"];
    for (const modeStr of modeSubContexts) {
      if (!validModes.includes(modeStr)) {
        continue;
      }
      const mode = modeStr;
      if (filterMode !== void 0 && filterMode !== mode) {
        continue;
      }
      if (filterMode === void 0 && isProduction !== void 0) {
      ...
      for (const versionKey of versionKeys) {
        const fileDesignator = new import_PersistedFileDesignator.PersistedFileDesignator(versionKey, blobDriver, modeContexts);
  ...
  async #checkEntry(fileDesignator, options) {
    try {
      ...
      if (currentVersion !== void 0) {
        if (header.minApplicableSoftwareVersion !== void 0 && currentVersion < header.minApplicableSoftwareVersion) {
          return;
        }
        if (header.maxApplicableSoftwareVersion !== void 0 && currentVersion > header.maxApplicableSoftwareVersion) {
          return;
        }
      }
      return {
  ...
  async fileDesignatorForUpdate(filename) {
    if (!OTA_FILENAME_REGEX.test(filename)) {
      throw new import_general.ImplementationError(`Invalid OTA filename format: ${filename}`);
    }
    if (this.#storage === void 0) {
      await this.construction;
    }
    const fileDesignator = new import_PersistedFileDesignator.PersistedFileDesignator(filename, this.#storage);
    if (!await fileDesignator.exists()) {
  ...
  async delete(options) {
    ...
    if (mode === void 0 && isProduction !== void 0) {
    ...
    if (vendorId !== void 0 && productId !== void 0 && mode !== void 0) {
      const vendorHex2 = vendorId.toString(16);
      const productHex = productId.toString(16);
      const modeContexts = [...baseContexts, vendorHex2, productHex, mode];
      const versionKeys = await blobDriver.keys(modeContexts);
      let deletedCount2 = 0;
      for (const versionKey of versionKeys) {
        const fd = new import_PersistedFileDesignator.PersistedFileDesignator(versionKey, blobDriver, modeContexts);
        await fd.delete();
        deletedCount2++;
      }
      if (deletedCount2 > 0) {
        logger.info(`Deleted ${deletedCount2} OTA file(s) for ${vendorHex2}.${productHex}.${mode}`);
      }
    ...
    let deletedCount = 0;
    const validModes = ["prod", "test", "local"];
    const modesToDelete = mode !== void 0 ? [mode] : validModes;
...
//# sourceMappingURL=DclOtaUpdateService.js.map
