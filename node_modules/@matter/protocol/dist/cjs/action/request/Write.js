...
var import_access_control = require("@matter/types/clusters/access-control");
 ...
 */
const AclClusterId = import_access_control.AccessControl.id;
const AclAttributeId = import_access_control.AccessControl.attributes.acl.id;
const AclExtensionAttributeId = import_access_control.AccessControl.attributes.extension.id;
...
function Write(optionsOrData, ...data) {
  ...
  function reifyData(data2) {
    ...
    for (const specifier of attributes) {
      ...
      const tlv = (0, import_types.TlvOfModel)(attribute);
      ...
      Array.isArray(value) && tlv instanceof import_types.ArraySchema && !isAclOrExtensionPath({ clusterId, attributeId })) {
        writeRequests.push(
          ...tlv.encodeAsChunkedArray(value, { forWriteInteraction: true }).map(({ element: data3, listIndex }) => ({
            path: {
              ...prototype.path,
