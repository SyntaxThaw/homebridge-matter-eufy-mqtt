...
class CaseServer {
  ...
  async onNewExchange(exchange) {
    ...
    try {
      ...
      if ((0, import_general.causedBy)(
        error,
        import_FabricManager.FabricNotFoundError,
        import_SecureChannelMessenger.ChannelStatusResponseError,
        import_PeerCommunicationError.TransientPeerCommunicationError,
        import_SessionManager.ShutdownError
      )) {
        logger.error(messenger.via, "Error establishing CASE session:", import_general.Diagnostic.errorMessage(error));
      } else {
      ...
      if ((0, import_general.causedBy)(error, import_FabricManager.FabricNotFoundError)) {
        await messenger.sendError(import_types.SecureChannelStatusCode.NoSharedTrustRoots);
      } else if (!(0, import_general.causedBy)(error, import_SecureChannelMessenger.ChannelStatusResponseError, import_PeerCommunicationError.PeerUnresponsiveError)) {
  ...
  async #handleSigma1(messenger) {
    logger.info(messenger.via, "Pairing request", import_Mark.Mark.INBOUND, import_general.Diagnostic.via(messenger.channelName));
  ...
  async #generateSigma2(cx, channel) {
    ...
    await crypto.verifyEcdsa((0, import_general.PublicKey)(peerPublicKey), peerSignatureData, new import_general.EcdsaSignature(peerSignature));
